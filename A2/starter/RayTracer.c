...
void rtShade(struct object3D *obj, struct point3D *p, struct point3D *n, struct ray3D *ray, int depth, double a, double b, struct colourRGB *col)
{
    ...
    struct point3D *shadow_direction = newPoint(curr_ls->p0.px, curr_ls->p0.py, curr_ls->p0.pz);
    ...
    if (shadow_lambda > 0.0 && 1.0 > shadow_lambda) {
        tmp_col.R += R * 0.1; // (obj->alb.ra); // diffuse test
        tmp_col.G += G * 0.1; // (obj->alb.ra);
        tmp_col.B += B * 0.1; // (obj->alb.ra);
    } else {
        normalize(shadow_direction);
        double intensity = max(0.0, dot(n, shadow_direction));
        // if object lit on both side flip the normal
        if (obj->frontAndBack) {
            n->px = -n->px;
            n->py = -n->py;
            n->pz = -n->pz;
            intensity = max(intensity, dot(n, shadow_direction));
        }
        tmp_col.R += R * (0.1 + (obj->alb.rd * curr_ls->col.R * intensity));
        tmp_col.G += G * (0.1 + (obj->alb.rd * curr_ls->col.G * intensity));
        tmp_col.B += B * (0.1 + (obj->alb.rd * curr_ls->col.B * intensity));
    }
